...
class DormBranchController extends Controller
{
    ...
    public function dorm_branch_tbl(Request $request)
    {
        ...
        $all_records = DormBranch::count();

        if (empty($request->search)) {
            $s = DormBranch::offset($start_from)
            ...
            $s = DormBranch::wwhere('description', 'LIKE', "%{$search}%")
    ...
    public function edit(Request $request)
    {
        ...
        //return  $request->all();
        $d = DormBranch::leftJoin('dorm_types', 'dorm_types.id', 'dorm_branch.dorm_type')
            ->leftJoin('featureds', 'featureds.dorm_id', 'dorm_branch.id')
            ->where('dorm_branch.id', $request->id)
            ->select([
                'dorm_branch.*',
                'dorm_types.des as typedes',
                'featureds.status as fstat'
            ])->first();
    ...
    public function forapprove_dorm_branch(Request $request)
    {
        ...
        if ($state == 1) {
            $message = "Dorm Publish.";
        } else {
            $message = "Dorm Withold.";
        }
    ...
    public function set_featured(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $check_featured = Featured::where('dorm_id', $dorm_id)->first();
            ...
            if (empty($check_featured)) {
                ...
                if ($check_featured->status == 1) {
                    $check_featured->status = 0;
                    $check_featured->save();
                } else {
                    $check_featured->status = 1;
                    $check_featured->save();
                }
            }
