...
{
    /* Converter without centavos */
    public function toText($amt, $cur)
    {
        if (is_numeric($amt)) {
            #echo '' . number_format($amt, 0, '.', ',') . '';
            $sign = $amt > 0 ? '' : 'Negative ';
            $amtArr = explode('.', $amt);
            if (!isset($amtArr[1]) || $amtArr[1] <= 0) {
                $inWords = $sign . $this->toQuadrillions(abs($amtArr[0])) . $cur;
            } else {
                $inWords = $sign . $this->toQuadrillions(abs($amtArr[0])) . $cur . ' and ' . $this->toQuadrillions($amtArr[1]) . 'Centavos';
            }
            return $inWords;
        } else {
