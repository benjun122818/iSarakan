        ...
        <div class="card w-full bg-base-100 shadow-xl">
            <div class="card-body">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <h2 class="card-title">TMP Users</h2>
                    </div>
                        ...
                        <tbody>
                            <template v-for="(s, index) in users">
                                <tr>
                                    ...
                                    <td>{{ s.name }}</td>
                                    <td>{{ s.email }}</td>
                                    <td>{{ (s.user_id == 0 ? "For Approval" : "Subscribed") }}</td>

                                    <td style="width: 10%">
                                        <div class="join">
                                            <button class="btn btn-info btn-sm join-item" @click="approveSubs(s.id, index)">
...
export default {
    ...
    methods: {
        ...
        approveSubs(id, index) {

            axios
                .post("/admin/approve-subscription", {
                    id: id
                ...
                .then((response) => {
                    ...
                    if (response.data.statcode == 1) {
                        ...
                        this.users[index].user_id = response.data.user.id;
                    } else {
                        Swal.fire({
                            title: "Oops!",
